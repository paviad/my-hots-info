...
public partial class InitForm : Form {
    ...
    private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e) {
        ...
        var config = _onlineMode
            ? CASCConfig.LoadOnlineStorageConfig(arg, "us")
            : CASCConfig.LoadLocalStorageConfig(arg);

        if (_onlineMode) {
            using (var sb = new SelectBuildForm(config)) {
