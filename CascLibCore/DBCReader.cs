...
internal class DBCRow(DBCReader reader, byte[] data) {
    ...
    public T? GetField<T>(int field) {
        ...
        switch (Type.GetTypeCode(typeof(T))) {
            case TypeCode.String:
                ...
                retVal = Encoding.UTF8.GetString(reader.StringTable, start, len);
                return (T)retVal;
            case TypeCode.Int32:
...
internal class DBCReader : IEnumerable<KeyValuePair<int, DBCRow>> {
    ...
    private readonly Dictionary<int, DBCRow> m_index = [];
    ...
    public DBCRow? GetRow(int index) {
        return m_index.GetValueOrDefault(index);
    }
