...
public class DB3Row(DB3Reader reader, byte[] data) {
        ...
        fixed (byte* ptr = Data) {
            ...
            switch (Type.GetTypeCode(typeof(T))) {
                case TypeCode.String:
                    var start = BitConverter.ToInt32(Data, offset);
                    retVal = reader.StringTable.TryGetValue(start, out var str) ? str : string.Empty;
                    return (T)retVal;
                case TypeCode.SByte:
                    retVal = ptr[offset];
                    return (T)retVal;
...
public class DB3Reader : IEnumerable<KeyValuePair<int, DB3Row>> {
    ...
    private readonly SortedDictionary<int, DB3Row> m_index = [];
    ...
    public Dictionary<int, string> StringTable { get; }
