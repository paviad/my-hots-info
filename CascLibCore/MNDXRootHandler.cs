...
namespace CASCExplorer
{
    ...
    internal class MNDXRootHandler : RootHandlerBase
    {
        ...
        private readonly Dictionary<ulong, RootEntry> mndxData = new Dictionary<ulong, RootEntry>();

        private readonly Dictionary<int, CASC_ROOT_ENTRY_MNDX> mndxRootEntries =
            new Dictionary<int, CASC_ROOT_ENTRY_MNDX>();

        private readonly Dictionary<int, CASC_ROOT_ENTRY_MNDX> mndxRootEntriesValid;

        private readonly Dictionary<int, string> Packages = new Dictionary<int, string>();
        private readonly Dictionary<int, LocaleFlags> PackagesLocale = new Dictionary<int, LocaleFlags>();
        ...
        public MNDXRootHandler(BinaryReader stream, BackgroundWorkerEx worker)
        {
            ...
            mndxRootEntriesValid =
                new Dictionary<int, CASC_ROOT_ENTRY_MNDX>(); // mndxRootEntries.Where(e => (e.Flags & 0x80000000) != 0).ToList();
        ...
        public override void LoadListFile(string path, BackgroundWorkerEx worker = null)
        {
            ...
            var regex1 = new Regex("\\w{4}(?=\\.(storm|sc2)data)", RegexOptions.Compiled);
            var regex2 = new Regex("\\w{4}(?=\\.(storm|sc2)assets)", RegexOptions.Compiled);
        ...
        {
            var result = new MNDXSearchResult();
    ...
    internal class MARFileNameDB
    {
        ...
        private bool CheckNextPathFragment(MNDXSearchResult pStruct1C)
        {
            ...
            NameFragIndex =
                ((pStruct40.ItemIndex << 0x05) ^ pStruct40.ItemIndex ^ pStruct1C.SearchMask[pStruct40.CharIndex]) &
                NameFragIndexMask;
    ...
    {
        private readonly List<PATH_STOP> PathStops = new List<PATH_STOP>(); // Array of path checkpoints
        private readonly List<byte> SearchResult = new List<byte>();
