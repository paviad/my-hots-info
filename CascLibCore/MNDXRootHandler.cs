...
public class PATH_STOP {
    ...
    public int field_4 { get; set; } = 0;
    public int field_8 { get; set; } = 0;
    public int field_C { get; set; } = -1;
    public int field_10 { get; set; } = -1;
}

internal partial class MNDXRootHandler : RootHandlerBase {
    ...
    private readonly MARFileNameDB[] MarFiles = new MARFileNameDB[CASC_MAX_MAR_FILES];

    private readonly Dictionary<ulong, RootEntry> mndxData = [];

    private readonly Dictionary<int, CASC_ROOT_ENTRY_MNDX> mndxRootEntries =
        [];

    private readonly Dictionary<int, CASC_ROOT_ENTRY_MNDX> mndxRootEntriesValid;

    private readonly Dictionary<int, string> Packages = [];
    private readonly Dictionary<int, LocaleFlags> PackagesLocale = [];
    ...
    public MNDXRootHandler(BinaryReader stream, BackgroundWorkerEx? worker) {
        ...
        var MarInfoOffset = stream.ReadInt32(); // Offset of the first MAR entry info
        var MarInfoCount = stream.ReadInt32(); // Number of the MAR info entries
        var MarInfoSize = stream.ReadInt32(); // Size of the MAR info entry
        var MndxEntriesOffset = stream.ReadInt32();
        var MndxEntriesTotal = stream.ReadInt32(); // Total number of MNDX root entries
        var MndxEntriesValid = stream.ReadInt32(); // Number of valid MNDX root entries
        var MndxEntrySize = stream.ReadInt32(); // Size of one MNDX root entry
        ...
        for (var i = 0; i < MarInfoCount; i++) {
            ...
            var marInfo = stream.Read<MARInfo>();
        ...
        for (var i = 0; i < MndxEntriesTotal; i++) {
            var entry = new CASC_ROOT_ENTRY_MNDX();
        ...
        mndxRootEntriesValid =
            []; // mndxRootEntries.Where(e => (e.Flags & 0x80000000) != 0).ToList();
    ...
    public override IEnumerable<RootEntry> GetAllEntries(ulong hash) {
        if (mndxData.TryGetValue(hash, out var rootEntry)) {
    ...
    public override void LoadListFile(string path, BackgroundWorkerEx? worker = null) {
        ...
        foreach (var result in MarFiles[0].EnumerateFiles()) {
            ...
            var match1 = regex1.Match(result.FoundPath);
            var match2 = regex2.Match(result.FoundPath);
    ...
    private CASC_ROOT_ENTRY_MNDX FindMNDXInfo(string path, int dwPackage) {
        var result = new MNDXSearchResult { SearchMask = path[(Packages[dwPackage].Length + 1)..].ToLower() };

        var marFile1 = MarFiles[1];
    ...
    private CASC_ROOT_ENTRY_MNDX FindMNDXInfo2(string path, int dwPackage) {
        var result = new MNDXSearchResult { SearchMask = path };

        var marFile2 = MarFiles[2];
    ...
    private int FindMNDXPackage(string fileName) {
        ...
        foreach (var package in Packages) {
            var pkgName = package.Value;
            var pkgNameLen = pkgName.Length;
    ...
    [GeneratedRegex(@"\w{4}(?=\.(storm|sc2)data)")]
    private static partial Regex Regex1();
    [GeneratedRegex(@"\w{4}(?=\.(storm|sc2)assets)")]
    private static partial Regex Regex2();
}

internal class MARFileNameDB {
    private const int CASC_MAR_SIGNATURE = 0x0052414d; // 'MAR\0'
    private readonly int field_214;
    private readonly TSparseArray FileNameIndexes;
    private readonly TBitEntryArray FrgmDist_HiBits;
    private readonly byte[] FrgmDist_LoBits;
    private readonly TNameIndexStruct IndexStruct_174;
    private readonly int NameFragIndexMask;
    private readonly NAME_FRAG[] NameFragTable;
    private readonly MARFileNameDB NextDB;

    private readonly TSparseArray Struct68_00;
    private readonly TSparseArray Struct68_D0;

    private readonly byte[] table_1BA1818 = [
    ...
    public IEnumerable<MNDXSearchResult> EnumerateFiles() {
        var pStruct1C = new MNDXSearchResult();
    ...
    public bool FindFileInDatabase(MNDXSearchResult pStruct1C) {
        var pStruct40 = pStruct1C.Buffer;
    ...
    private bool CheckNextPathFragment(MNDXSearchResult pStruct1C) {
        var pStruct40 = pStruct1C.Buffer;

        // Calculate index of the next name fragment in the name fragment table
        var NameFragIndex = ((pStruct40.ItemIndex << 0x05) ^ pStruct40.ItemIndex ^ pStruct1C.SearchMask[pStruct40.CharIndex]) &
                            NameFragIndexMask;
        ...
        //
        // HOTS: 1957A0E
        var CollisionIndex = sub_1959CB0(pStruct40.ItemIndex) + 1;
        ...
        do {
            ...
            if (Struct68_D0.Contains(pStruct40.ItemIndex)) {
                ...
                // HOTS: 1957A83
                var SaveCharIndex = pStruct40.CharIndex;

                // Get the name fragment offset as combined value from lower 8 bits and upper bits
                var FragOffs = GetNameFragmentOffsetEx(pStruct40.ItemIndex, HiBitsIndex);
    ...
    private bool EnumerateFiles(MNDXSearchResult pStruct1C) {
        ...
        for (;;) {
            ...
            if (pStruct40.ItemCount == pStruct40.NumPathStops) {
                var pLastStop = pStruct40.GetPathStop(pStruct40.NumPathStops - 1);
                var CollisionIndex = sub_1959CB0(pLastStop.ItemIndex) + 1;
            ...
            // HOTS: 19595BD
            var pPathStop = pStruct40.GetPathStop(pStruct40.ItemCount);
    ...
    private bool sub_1957B80(MNDXSearchResult pStruct1C, int dwKey) {
        ...
        // HOTS: 1957B95
        for (;;) {
            var pNameEntry = NameFragTable[edi & NameFragIndexMask];
    ...
    private bool sub_1958B00(MNDXSearchResult pStruct1C) {
        var pStruct40 = pStruct1C.Buffer;
        var pbPathName = Encoding.ASCII.GetBytes(pStruct1C.SearchMask);

        var ItemIndex = pbPathName[pStruct40.CharIndex] ^ (pStruct40.ItemIndex << 0x05) ^ pStruct40.ItemIndex;
        ItemIndex = ItemIndex & NameFragIndexMask;
        if (pStruct40.ItemIndex == NameFragTable[ItemIndex].ItemIndex) {
            // HOTS: 1958B45
            var FragmentOffset = NameFragTable[ItemIndex].FragOffs;
        ...
        for (;;) {
            if (Struct68_D0.Contains(pStruct40.ItemIndex)) {
                ...
                var FragOffs = GetNameFragmentOffsetEx(pStruct40.ItemIndex, var_4);
                if (NextDB != null) {
                    // HOTS: 1958CCB
    ...
    private void sub_1958D70(MNDXSearchResult pStruct1C, int arg_4) {
        ...
        // HOTS: 1958D84
        for (;;) {
            var pNameEntry = NameFragTable[arg_4 & NameFragIndexMask];
    ...
    private int sub_1959CB0(int dwItemIndex) {
        ...
        var eax = Struct68_00.GetArrayValue_38(dwKeyShifted) >> 9;
        var esi = (Struct68_00.GetArrayValue_38(dwKeyShifted + 1) + 0x1FF) >> 9;
        dwItemIndex = esi;
...
public class TBitEntryArray : List<int> {
    ...
    private readonly int EntryBitMask;
    ...
    public new int this[int index] {
        get {
            ...
            if (dwEndBit > 0x20) {
                dwResult = (base[dwItemIndex + 1] << (0x20 - dwStartBit)) |
                           (int)((uint)base[dwItemIndex] >> dwStartBit);
            }
            else {
...
public class TSparseArray {
    private readonly int[] ArrayDwords_38;
    private readonly int[] ArrayDwords_50;
    private readonly TRIPLET[] BaseValues; // Array of base values for item indexes >= 0x200
    private readonly int[] ItemBits; // Bit array for each item (1 = item is present)
    ...
    public int TotalItemCount { get; } // Total number of items in the array
    public int ValidItemCount { get; } // Number of present items in the array
    ...
    public int GetItemValue(int index) {
        var pTriplet =
            ...
            BaseValues[index >> 0x09];
        var BaseValue = pTriplet.BaseValue;
...
public class TNameIndexStruct {
    private readonly TSparseArray FragmentEnds;
    private readonly byte[] NameFragments;
    ...
    public bool CheckAndCopyNameFragment(MNDXSearchResult pStruct1C, int dwFragOffs) {
        ...
        if (FragmentEnds.TotalItemCount == 0) {
            var szSearchMask = pStruct1C.SearchMask;
...
public class SearchBuffer {
    private readonly List<PATH_STOP> PathStops = []; // Array of path checkpoints
    private readonly List<byte> SearchResult = [];

    public int ItemIndex { get; set; } // Current name fragment: Index to various tables
    public int CharIndex { get; set; }
    public int ItemCount { get; set; }

    public CASCSearchPhase SearchPhase { get; private set; } =
        CASCSearchPhase.Initializing; // 0 = initializing, 2 = searching, 4 = finished
...
public class MNDXSearchResult {
    ...
    public SearchBuffer Buffer { get; }
