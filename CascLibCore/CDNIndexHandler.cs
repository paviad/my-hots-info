...
public class CDNIndexHandler {
    ...
    public static readonly CDNCache Cache = new("cache");
    ...
    public static Stream OpenConfigFileDirect(CASCConfig cfg, string key) {
        var file = cfg.CDNPath + "/config/" + key[..2] + "/" + key.Substring(2, 2) + "/" + key;
    ...
    public IndexEntry? GetIndexInfo(MD5Hash key) {
        if (!CDNIndexData.TryGetValue(key, out var result)) {
    ...
    public Stream OpenDataFile(IndexEntry entry) {
        ...
        var file = config.CDNPath + "/data/" + archive[..2] + "/" + archive.Substring(2, 2) + "/" +
                   archive;
    ...
    public Stream OpenDataFileDirect(MD5Hash key) {
        ...
        worker?.ReportProgress(0, $"Downloading \"{keyStr}\" file...");

        var file = config.CDNPath + "/data/" + keyStr[..2] + "/" + keyStr.Substring(2, 2) + "/" + keyStr;
        var url = "http://" + config.CDNHost + "/" + file;
    ...
    private void ParseIndex(Stream stream, int i) {
        ...
        for (var j = 0; j < count; ++j) {
            ...
            var entry = new IndexEntry { Index = i,
                Size = br.ReadInt32BE(),
