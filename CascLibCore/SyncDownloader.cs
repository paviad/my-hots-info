...
public class SyncDownloader(BackgroundWorkerEx? progressReporter) {
    ...
    public static CacheMetaData? GetMetaData(string url, string file) {
        try {
            var request = WebRequest.CreateHttp(url);
            request.Method = "HEAD";
    ...
    private void CopyToStream(Stream src, Stream dst, long len) {
        ...
        int count;
        do {
            if (progressReporter is { CancellationPending: true }) {
                return;
            }
